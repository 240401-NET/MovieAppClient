import React, {useState, useEffect, createContext, useContext, AudioHTMLAttributes} from 'react';
import { useNavigate } from 'react-router-dom';
import { UserLogin, UserLogout, UserRegistration } from '../services/UserServices';
...
export const AuthenticationProvider = ( children: React.ReactNode )  => {
        ...
        await UserLogin(username, password)
            .then ((res) => {
                if(res) {
                    ...
                    localStorage.setItem("token", "customToken" + res.statusText + res.url);
                    setUser(username);
    ...
    return (
        <AuthenticationContext.Provider value={{loginUser, isLoggedIn, user, token, registerUser, logoutUser}}>
